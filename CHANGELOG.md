...
- RSS connection: fixed an issue where the connection would throw an error if it parses a post that does not have a last published date, which is used to set the `min_id` for the connection. We now fallback to the latest published `link` if the published date is not available.

### Changed

- Minor dependency updates and typing tweaks.
